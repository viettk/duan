...
public class LoginRest {
    ...
    public ResponseEntity<Object> authenticateUser(@Valid @RequestBody LoginRequest loginRequest) {
        ...
        Optional<StaffEntity> staff = staffRepository.findByEmail(loginRequest.getUsername());
        if(staff.isPresent()){
            staff.get().setToken(jwt);
            return ResponseEntity.ok(staff);
        }else {
            CustomerEntity customer = customerRepository.findByEmail(loginRequest.getUsername()).get();
